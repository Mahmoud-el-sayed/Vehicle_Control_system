...
//define ON and off using enum
enum state{ ON , OFF };

// struct include by vechile state
struct engine{
	int speed ;
	int room_temp;
#if (WITH_ENGINE_CONTROLLER==1)
...
int main(void) {
	setbuf(stdout,NULL);
	char input;
	//initial values for engine
	intial_values();
	//take input from user to make engine OFF or ON or quit
	input= take_input();
	//loop for input and exit in case quit only
	while(1){
		switch (input){
		case 'a':
...
//function to read traffic_color_sensor_value
char traffic_color_sensor(void){
	char input3;
	do {
...
//function to read  room_temp_sensor_value
void room_temp_sensor(void){
...
//function to read  engine_temp_sensor_value
#if (WITH_ENGINE_CONTROLLER==1)
void engine_temp_sensor(void){
...
//function to read  speed_test_value
